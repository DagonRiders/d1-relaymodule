...
do
  ...
  local connect = function(host, port)
    ...
    _fd:on("reconnection", function(socket, string)
      print("Reconnection:" .. string)
      -- Connect failed, try to reconnect after ten seconds has passed
      tmr.alarm(2, 10000, 0, function()
        print("Attempting to reconnect...")
        _fd:connect(port or REDIS_PORT, host)
      end)
    end)
    _fd:on("disconnection", function(socket, string)
      print("Disconnection:" .. string)
    end)
